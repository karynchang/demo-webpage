!function() {
  ...
  new Vue({
      ...
      submit: function() {
        ...
        if (void 0 !== this.person.area) {
          ...
          fetch(this.formAction + "?" + n, { method: "POST" }).catch(function(n) {
            alert(submitdata), t.name = "",t.phone="", t.person = {}, t.persons ={};
      ...
      // if(this.name.length >= 2 && this.name.length < 7 ){  // 2 <= 名字長度 <= 6
        if (void 0 === this.persons[this.name]) {
          ...
          fetch(i, { method: "POST" })
            ...
            if (void 0 === deptName ) {
              deptGroup.style.display = 'block';// 如果名字不为空，则显示部门字段
              ...
              deptGroup.style.display = 'none'; // 如果名字为空，则隐藏部门字段
    ...
    wrongData(){
      ...
      Swal.fire({
        title: '請確認資料錯誤原因',
        html: '<div style="text-align: left;">如果只有少部分資料錯誤，<br>請點選 修改資料。<br><br>如果資料全部錯誤，<br>請點選 改用電話查詢。',
        icon: 'warning',
        showCancelButton: true,
